...
function applyChanges() {
  ...
  // Remove specs grid
  pd = pd.replace(/<div class="detail-specs-grid" id="detailSpecsGrid">[\s\S]*?<\/div>\s*<\/div>/, '</div>');
  
  // Remove detailStars
  pd = pd.replace(/<div class="detail-stars" id="detailStars">[\s\S]*?<\/div>/, '');
  
  // Update initProductDetailUI tabs rendering
...
async function uploadReviewImage(fileInput, index) {
  ...
  admin = admin.replace(scriptEnd, newFunctions);
  fs.writeFileSync('admin.html', admin, 'utf-8');
  console.log('Successfully re-applied changes.');
}
